...
export function PendingCommentItem({ comment }: PendingCommentItemProps) {
  ...
  const shouldShowExpandOption = commentContent.length > 80;

  return (
    <div className="flex flex-col space-y-2 rounded-md border p-3 comment-card-hover">
      ...
      <div>
        <p className={`text-sm ${!isExpanded && shouldShowExpandOption ? 'line-clamp-2' : ''}`}>
          {commentContent}
        </p>
